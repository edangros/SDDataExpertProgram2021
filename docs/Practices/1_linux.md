# 시스템 재구축 주말 - 1.리눅스 가상머신 설치 및 Remote 원격제어 구축
...
이번 주말동안에는 OS설치 및 원격접속 구축부터 재시작하고, 하둡 클러스터 및 Spark를 위한 파이썬/스칼라 개발환경까지 재시작하여 워크플로우를 확립, 이후 학습과 프로젝트 진행에 대비하는 것이 목표이다.
...
### 2.4 등록된 키를 통한 SSH연결, 비밀번호를 통한 연결 차단
